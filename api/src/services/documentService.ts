...
const validateAndFixCURP = (curp: string): FieldValidationResult => {
...
const validateAndFixRFC = (rfc: string): FieldValidationResult => {
...
const validateAndFixCLABE = (clabe: string): FieldValidationResult => {
...
const validateAndFixVIN = (vin: string): FieldValidationResult => {
...
const validateAndFixPlacas = (placas: string): FieldValidationResult => {
...
export async function analyzeDocument(
  ...
  try {
    ...
    // For PDFs, we'd need to convert - for now, assume images
    // In production, add PDF conversion with pdf-lib or similar
    
    const supportedTypes = ['image/png', 'image/jpeg', 'image/gif', 'image/webp'];
    const finalMimeType = supportedTypes.includes(mimeType) ? mimeType : 'image/png';

    const response = await openai.chat.completions.create({
      model: 'gpt-4o', // Best accuracy for OCR
      messages: [
        {
          role: 'user',
          content: [
    ...
    for (const [key, value] of Object.entries(parsed.extractedData || {})) {
      if (value && value !== 'N/A' && value !== 'No visible') {
